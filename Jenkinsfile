                ...
                script{
                      docker.withRegistry('https://216413260795.dkr.ecr.us-east-1.amazonaws.com', 'ecr:us-east-1:aws-credentials') {
                        withAWS(region: "${AWS_ECR_REGION}", credentials: 'aws-credentials') {
                            ...
                            sh("aws ecs register-task-definition --region ${AWS_ECR_REGION} --family ${AWS_ECS_TASK_DEFINITION} --execution-role-arn arn:aws:iam::216413260795:role/ecsTaskExecutionRolePolicy --requires-compatibilities EC2 --network-mode bridge --cpu 1024 --memory 512 --container-definitions file://${AWS_ECS_TASK_DEFINITION_PATH}")
